...
class ContiFormScreen extends ConsumerStatefulWidget {
  final int teamId;
  final int? contiId;

  const ContiFormScreen({super.key, required this.teamId, this.contiId});

  @override
  ConsumerState<ContiFormScreen> createState() => _ContiFormScreenState();
}

class _ContiFormScreenState extends ConsumerState<ContiFormScreen> {
  ...
  void initState() {
    super.initState();
    _isEdit = widget.contiId != null;
    if (_isEdit) {
      _loadConti();
    }
  }

  Future<void> _loadConti() async {
    final api = ref.read(apiClientProvider);
    final response = await api.get<SetlistDetailResponse>(
      '/teams/${widget.teamId}/setlists/${widget.contiId}',
  ...
  Future<void> _submit() async {
    ...
    if (mounted) {
      ...
      ref.invalidate(setlistsProvider(
          SetlistListParams(teamId: widget.teamId, setlistType: 'CONTI')));
      context.pop();
    }
  }
